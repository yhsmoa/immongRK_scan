...
// ✅ DB 정보 페이지 (페이지별 Supabase 테이블 안내)
app.get('/dbInfo', (req, res) => {
  res.sendFile(path.join(__dirname, 'dbInfo.html'));
});
...
app.post('/api/inventory/generate-discontinue-doc', async (req, res) => {
    try {
        ...
        if (!docXmlFile) {
            console.error('word/document.xml을 찾을 수 없습니다.');
            return res.status(500).json({ error: '템플릿 파일 구조가 올바르지 않습니다.' });
        }
...
app.post('/api/neworder/upload', upload.single('file'), async (req, res) => {
    try {
        ...
        for (const orderNumber in groupedData) {
            const existingOrder = await NewOrder.findOne({ 발주번호: orderNumber });
            if (existingOrder) {
                duplicateOrders.push(orderNumber);
            } else {
                newOrders.push(groupedData[orderNumber]);
        ...
        // 새로운 발주서만 저장
        if (newOrders.length > 0) {
            await NewOrder.insertMany(newOrders);
        }

        // 응답 메시지 구성
        let message = '';
        if (newOrders.length > 0) {
            message += `${newOrders.length}개의 신규발주서가 추가되었습니다.`;
        }
        if (duplicateOrders.length > 0) {
            message += `\n다음 발주서는 이미 존재합니다:\n${duplicateOrders.join(', ')}`;
        }
...
// Add after the existing '/api/orders' endpoint
// 신규 발주서 목록 가져오기
app.get('/api/neworders', async (req, res) => {
    try {
        const orders = await NewOrder.find().sort({ 입고예정일: 1, 물류센터: 1, 발주번호: 1 });
        res.json(orders);
    } catch (error) {
        console.error('신규 발주서 목록 조회 중 오류 발생:', error);
        res.status(500).json({ error: '신규 발주서 목록을 불러오는데 실패했습니다.' });
    }
});

// Add the newOrders delete endpoint
app.post('/api/neworders/delete', async (req, res) => {
    try {
        ...
        await NewOrder.deleteMany({ 발주번호: { $in: orderNumbers } });
        res.json({ message: '발주서가 성공적으로 삭제되었습니다.' });
    } catch (error) {
        console.error('신규 발주서 삭제 실패:', error);
        res.status(500).json({ error: '신규 발주서 삭제에 실패했습니다.' });
    }
});

// 신규 발주서 내보내기 API
app.post('/api/neworders/export', async (req, res) => {
    try {
        const { orderNumbers } = req.body;
        const query = orderNumbers && orderNumbers.length > 0 
            ? { 발주번호: { $in: orderNumbers } }
        ...
        let filename;
        if (orderNumbers && orderNumbers.length === 1) {
            filename = `신규발주리스트 (${orderNumbers[0]}).xlsx`;
        } else if (orderNumbers && orderNumbers.length > 1) {
            filename = `신규발주리스트 (${orderNumbers[0]} 외 ${orderNumbers.length - 1}건).xlsx`;
        } else {
            filename = `신규발주리스트 (전체).xlsx`;
        }
        ...
        console.error('신규 발주서 내보내기 중 오류:', error);
        res.status(500).json({ error: '서버 오류가 발생했습니다.' });
    }
});

// 신규 발주서 위치 정보 업데이트 API
...
// 신규 발주서를 기존 발주서로 등록하는 API
app.post('/api/neworders/register', async (req, res) => {
    try {
        const { orderNumbers } = req.body;
        
        if (!Array.isArray(orderNumbers) || orderNumbers.length === 0) {
            return res.status(400).json({ 
                message: '등록할 발주서가 선택되지 않았습니다.' 
            });
        }
        
        // 선택된 신규 발주서 가져오기
        const newOrders = await NewOrder.find({ 발주번호: { $in: orderNumbers } });
        
        if (newOrders.length === 0) {
            return res.status(404).json({ 
                message: '등록할 발주서를 찾을 수 없습니다.' 
            });
        }
        ...
        for (const newOrder of newOrders) {
            try {
                // 기존 발주서에 중복 체크
                const existingOrder = await Order.findOne({ 발주번호: newOrder.발주번호 });
                ...
                // 새 발주서 객체 생성 (NewOrder와 동일한 스키마 사용)
                const orderToSave = new Order({
                    발주번호: newOrder.발주번호,
                    입고예정일: newOrder.입고예정일,
                    물류센터: newOrder.물류센터,
                    상품수: newOrder.상품수,
                    발주수량: newOrder.발주수량,
                    확정수량: newOrder.확정수량,
                ...
                // 등록 성공 후 신규 발주서에서 삭제 (옵션)
                // await NewOrder.deleteOne({ 발주번호: newOrder.발주번호 });
            } catch (error) {
                console.error(`발주서 ${newOrder.발주번호} 등록 중 오류:`, error);
                errors.push(newOrder.발주번호);
            }
        }
        ...
        let message = '';
        if (registered > 0) {
            message += `${registered}개의 발주서가 성공적으로 등록되었습니다.`;
        }
        if (duplicates > 0) {
            message += `\n${duplicates}개의 발주서는 이미 존재합니다.`;
        }
        if (errors.length > 0) {
            message += `\n${errors.length}개의 발주서 등록 중 오류가 발생했습니다.`;
        }
